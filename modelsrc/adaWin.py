...
def window_partition(x, window_size):
    ...
    Returns:
        windows: (num_windows*B, window_size, window_size, C) Stack them together to form a long strip
    """
    B, H, W, C = x.shape
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size1, window_size2, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # Learnable weights of P_thermal
    ...
    def forward(self, x, y, P_thermal=None,mask=None):
        ...
        if P_thermal is not None:
            ...
            P_thermal = P_thermal.view(B_, -1, N, N)  # let P_thermal match (B_, C, N, N)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, semantic=None, P_thermal=None, p_saliency=None, alpha=4, threshold_sal=0.3,use_global=False):
        """
        p_saliency: (B, 1, H, W)
        alpha, threshold_sal: Dynamic window adjustment parameter
        use_global: Whether to enable local-global fusion
        """
        ...
        if self.shift_size > 0:
            ...
            if p_saliency is not None:
                # p_saliency: (B,1,H,W)， shift dims=(2,3)
                shifted_sal = torch.roll(p_saliency,shifts=(-self.shift_size, -self.shift_size),dims=(2, 3))
            else:
                shifted_sal = None
        else:
            shifted_x = x
        ...
        # ============== Before partitioning, perform window_partition on p_saliency and calculate the average ==============
        mean_sal = None
        if shifted_sal is not None:
            ...
            sal_windows = window_partition(sal_4d, self.window_size1)  # (nW*B, wsize1, wsize1, 1)
        ...
        if mean_sal is not None:
            # Calculate the global average significance of all Windows
            global_mean = mean_sal.mean().item()
            base_size = self.window_size2
            # The uniform dynamic window size is determined based on the global average significance
            if global_mean > threshold_sal:
                dyn_size = max(base_size - 2, 4)
            else:
                dyn_size = min(base_size + 2, 8)
            # Use dyn_size to uniformly block shifted_y
        ...
        # ---------------------------
        # The local-global association balance part
        if use_global:
            y_global = shifted_y.mean(dim=(1, 2), keepdim=True)  # shape: (B, 1, 1, C)
            num_windows_per_img = y_windows.shape[0] // B
            fused_y_windows = []
            for i in range(y_windows.shape[0]):
                b = i // num_windows_per_img
                patch = y_windows[i]  # (N_patch, C)，其中 N_patch = dyn_size * dyn_size
                global_patch = y_global[b].expand(patch.shape[0], C)  # (N_patch, C)
                fused_patch = 0.5 * patch + 0.5 * global_patch
                fused_y_windows.append(fused_patch)
            y_windows = torch.stack(fused_y_windows, dim=0)  # (nW*B, N_patch, C)
        # ---------------------------

        if P_thermal.dim() == 3:
            B, HW, C = P_thermal.shape
            P_thermal = P_thermal.transpose(1, 2).view(B, C, H, W)  # (B, C, H, W)

        if P_thermal is not None:
            P_thermal = F.interpolate(P_thermal, size=(H, W), mode='bilinear', align_corners=False)  # (B, C, H, W)
            P_thermal = window_partition(P_thermal.permute(0, 2, 3, 1),self.window_size1)  # (num_windows * B, window_size, window_size, C)
            P_thermal = P_thermal.view(-1, self.window_size1 * self.window_size1, C)  # (num_windows * B, N, C)
